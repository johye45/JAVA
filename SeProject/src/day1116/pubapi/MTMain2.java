...
public class MTMain2 extends JFrame{
	...
	SAXParser saxParser;
	Thread loadThread; //네트워크상에서 xml을 불러올때 사용할 쓰레드, 버튼을 누를때마다 인스턴스생성하여
	//네트워크 업무 시키자
	...
	public MTMain2() {
		...
		bt.addActionListener((e)->{
			...
			//아래의 코드는 버튼 누를때 시점으로 , 버튼을 계속 눌러도 에러 안나게
	...
	public void loadXML() { //이 명칭이 더 직관적일듯합니다
        ...
        try {
			...
			urlBuilder.append("&" + URLEncoder.encode("mntnNm","UTF-8") + "=" + URLEncoder.encode(t_name.getText(), "UTF-8")); /*검색어를 정해두지 않고 받아오기*/
	...
	public void parseData() {
		...
		try {
			...
			//파싱이 끝나면 TableModel의 벡터객체를 파싱한 결과로 얻은 벡터로 대체해버리면 됨
